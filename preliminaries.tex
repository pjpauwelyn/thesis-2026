...
\label{sec:prelim-intro}

We covered RAG architectures, ontologies, and multi-agent systems and reviewed their literature. In this chapter we evaluate the performance of the baseline pipeline~\citep{elbaff2025earthobservationrag} and analyse where it falls short. We describe how we tested it (\S\ref{sec:testing-methodology}), introduce the four recurring failure patterns with examples (\S\ref{sec:failure-patterns}), look at the retrieval context the generation agent receives (\S\ref{sec:generation-context}), trace the failures to their likely root causes (\S\ref{sec:root-causes}), and lay out the design decisions (\S\ref{sec:design-decisions}) behind the multi-agent ontology-enhanced approach described in Chapter~\ref{chap:approach}.
...
This is the most concerning example because it shows how multiple shortcomings compound into a misleading answer.
...
Unlike the baseline, the context passed to the generation agent is filtered against the query's semantic requirements rather than assembled by similarity alone.
...
This next chapter presents the full pipeline design.
