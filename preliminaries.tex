...
\paragraph{Query-adaptive ontologies.} For each question, we generate a lightweight ontology from natural language ~\citep{berant2013semantic}: a set of typed attributes (spatial scope, temporal scope, sensor, biophysical variable, application domain), each assigned an importance score. A static, pre-built ontology covering all of EO would be large, difficult to maintain, and blind to query-specific importance. Our ontologies are generated dynamically and contain only the attributes relevant to the current question.
