\begin{comment}
- chapter overview needs a change (refer to red comment in screenshot to undertsand how it needs to be replaced and better formulated.

- in all tables, figures and graphs we need to have the same consistent namings of the models, these all need to be changed and made clear if possible (for the graphs that need remaking, flag this clearly to me)

- figure 6.1 can be enlrged to more fit the width of the screen

- again in 6.5, the significance tests are on overall scores, this needs to be mentioned too here.

- the full chapter name should be changed to "experiments and results"

- between 6.1 and 6.2 we need to add a new section called "experimental settings" outlining the specifics: 70 questions, AQL, open alex api, models used and dates and features, temperatures ect..

- 6.1, "overview" section can have a linking sentence for flow.

- in 6.6 we can mention the extra experiment: extended the current pipeline to also include full-text snippets from each paper (results section, methodology), ect, when available. we ran this experiment with both mistral small and mistral large (to see wether a larger model could handle the bigger text load, expecially at refinement step) this proved to improve quantitive precision metric but then degraded all other metrics, specifically those from the 4 failure modes. this shows context engineering is a tricky approach, and our current pipeline is still fragile to prompt overloading.
this can be mentioned brifely and correctly integrated into section 6.6 but should just be as extra "proof" or experiment and not a full formal definition.
\end{comment}
...
pipeline leads on all criteria; the two-step baseline scores lowest overall.}
...
4 & Two-Step RAG Baseline & 5.51 / 10 & Scores lowest overall \\
\bottomrule
\end{tabular}
\end{table}
