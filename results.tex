...
Chapter~\ref{chap:evaluation} described the evaluation setup: four pipeline variants, 70~Earth observation questions, and Claude Sonnet~4.6 as an LLM-as-a-judge scoring eight criteria on a 1--10 scale. The judge evaluated each question-answer pair independently, without knowing which pipeline produced the answer. This gives 280~evaluations (70~questions $\times$ 4~pipelines) and 2{,}240 individual criterion scores. An earlier round using GPT-4.1-mini as judge is reported in Section~\ref{sec:initial-framework}; the Claude-based evaluation is the primary evidence base.
...
by Claude Sonnet~4.6 (mean of eight criteria, 1--10 scale).}
...
\caption{Heatmap of mean criterion scores across all four pipelines. The ontology-enhanced pipeline shows the deepest green shading across all criteria. Quantitative precision forms a visually distinct cool band for every pipeline.}
...
Bold values mark the highest score in each criterion column.}
...
On the other end, the two-step baseline loses to zero-shot on 66 of 70~questions (94\%). As Figure~\ref{fig:per-question} shows, the two-step baseline line runs consistently below the zero-shot line, with only 4~questions where the two-step baseline marginally exceeds zero-shot (all by less than 0.5 points). The degradation from the two-step refinement is systematic, not driven by a few bad cases.
...
($n = 70$). All comparisons are significant at $p < 0.001$. Effect sizes
($r$) are computed as $r = Z / \sqrt{N}$ following~\citet{zheng2023judging}.}
