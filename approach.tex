\begin{comment}
- flow at start of chapter 4 needs to be better, between the sections.
- dont mention aql results or open alex api (call it external data/documents for now) in graph and other places before they are formally introduced.

- each design decision needs to be backed up/ explained. either referencing a paper or giving a clear and correct reasoning behind it. this concerns: (1) "4-8 pairs" (resoning for this is that we want the ontology to be somewhat consistent, include as many dimensions as possible, which is not always possible given a simpler question, but also be able to have several value pairs per dimension for complex questions but not too many pairs either). (2) the "five mandatory dimensions" (this is a direct reference to the relevant paper about EO). (3) centrality scoring (this can be either explained as a way to enrich the ontology, make it more useful for the following agents, potentially a paper can be referenced here if relevant and applicable). (4) "four labelled sections" (resoning behind this is to mirror a similar structure as the el baff model (per document then topics) but then also add caveats and gaps section to address the scope awareness/context misalignment failure mode from the baseline.
...
\caption{Architecture of the ontology-enhanced multi-agent RAG pipeline. Light blue boxes represent existing two-step baseline components; dark blue boxes represent the three new agents.}
\label{fig:pipeline}
\end{figure}
...
\item[Spatial scope:] Geographic scope or resolution (e.g., boreal forest, Arctic, 1~km resolution).
\item[Temporal scope:] Time period or temporal resolution (e.g., decadal trends, 2000--present).
...
The ontology extraction prompt is shown in Listing~\ref{lst:ontology-prompt-excerpt}.
...
\item \textbf{Validation checklist.} Before returning, the model checks whether a domain expert could answer the question using only the extracted pairs.
\end{itemize}
...
In Earth observation, documents are almost never fully relevant or fully irrelevant. A study of SAR biomass in temperate forests is highly relevant for the sensor and variable dimensions but irrelevant for the spatial scope \texttt{boreal\_forest}. Instead of a binary accept/reject filter, the agent evaluates each document along the dimensions defined by the ontology. A document that partially matches is included with an explicit geographic caveat; a document that matches no high-centrality attribute is simply left out of all topics. This is especially important in EO because the literature for any given variable-sensor-region combination is sparse~\citep{aschbacher2020earth}. Excluding all partial matches would often leave the context empty.
...
Listing~\ref{lst:refinement-prompt-excerpt} shows the key sections of the refinement prompt.
...
\item \textbf{No-ontology variant.} For comparison, the ontology input is replaced with a note that none is provided and the selection criterion changes to ``prefer items directly relevant to the user question.'' Everything else stays identical, so any difference is attributable to the ontology.
\end{itemize}
...
This ordering ensures that retrieved, scope-tagged evidence dominates quantitative claims and geographic context, while the draft contributes general background that the retrieval corpus may not cover.

\subsection{Prompt Design}
\label{subsec:generation-prompt-design}

Listing~\ref{lst:generation-prompt-excerpt} shows the key sections of the generation refinement prompt.
...
The complete generation refinement prompt is provided in Listing~\ref{lst:generation-prompt} (Appendix~\ref{app:prompt-generation}).
