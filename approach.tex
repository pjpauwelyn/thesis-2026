...
\caption{Architecture of the ontology-enhanced multi-agent RAG pipeline. Grey boxes represent existing two-step baseline components; coloured boxes represent the three new agents.}
\label{fig:pipeline}
\end{figure}
...
3 & Spatial scope & spatial\_qualifier & boreal\_forest & 0.9 \\
...
7 & Temporal scope & temporal\_qualifier & multi-temporal\_analysis & 0.5 \\
\bottomrule
\end{tabular}
\end{table}

The target variable (AGB, 1.0), the specific sensor (Sentinel-1 C-band, 0.9), the geographic constraint (boreal forest, 0.9), and the key accuracy concern (C-band saturation, 0.9) all receive high centrality. The temporal scope receives 0.5 because the question does not stress any specific time period.
...
The no-ontology variant serves as the primary ablation: if the ontology-enhanced pipeline outperforms it, the improvement is attributable to ontology-guided semantic reasoning rather than to the general prompt improvements both variants share.
