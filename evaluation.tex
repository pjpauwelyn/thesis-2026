...
The final framework replaces GPT-4.1-mini with Claude Sonnet~4.6 and expands to eight criteria on a 1--10 scale, avoiding the score saturation that limited the initial framework. We switched to Claude Sonnet~4.6 to remove self-enhancement bias.
...
For each question-answer pair, the judge receives the system prompt with criteria definitions and score range descriptions, followed by the question and the answer. It returns eight integer scores. Each pair is evaluated on its own. This single-answer grading approach avoids position bias~\citep{zheng2023judging}. The Directness criterion explicitly penalises unnecessary verbosity.
