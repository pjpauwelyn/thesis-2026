\begin{comment}
- in 5.2, we say "we generated" the 70 questions but these question were taken from the el baff paper, and are re-used for comparative sake. this needs to be made clear el baff generated them, and we are simply re-using them as is.

- in 5.2, we don't need to go deep on each type of question, we can mention these but we should'nt have seperate bullet points for each type, this needs to be summarised and cut down.

- in 5.3:
- remove mistral small 24B mention
- this whole section needs to be more consistent, formal and straight forward.
- every model should be explained concistently and clearly. (same terms pipeline=system)
- we dont ned to re-explain things we already explained, we just need to list clearly the four models we will be evaluating and their differences. "we prompt our pipeline...", keeping it concise and direct.
- we need to change the naming (as previously mentioned for the two baselines, these need to be consistent). we can call "ontology enhanced" pipeline "our pipeline" and the "no-ontolgy variant" can be "our pipeline with no ontology"
- refer to the screenshot i will give to understand the red comments and how they relate to these points.

- from this point on, the naming of the different models should stay consistent everywher (graphs, scores, and other places)

- in 5.5, we need to mention the significance test are on overall scores not metric specific. this can be a small mention, no need for longer explanation.
\end{comment}
...
\item[Two-step baseline.] The pipeline described by~\citet{elbaff2025earthobservationrag}: a zero-shot draft refined using raw context from the EO knowledge graph. It uses no ontology, no intermediate document assessment, and the original baseline generation prompt.

\item[No-Ontology variant.] This variant uses the same improved Refinement and Generation Agent prompts as the ontology-enhanced pipeline, but the Refinement Agent receives no ontology. It evaluates documents based on the question text alone. This version isolates the ontology's contribution from prompt engineering improvements.
