...
This chapter describes how we evaluate the four pipeline variants introduced in Chapter~\ref{chap:approach}. We use a dataset of 70~Earth observation (EO) questions and a large language model (LLM)-as-a-judge framework with eight criteria scored on a 1--10 scale. We test for significant differences using non-parametric methods. The chapter first describes the evaluation dataset (\S\ref{sec:dataset}) and the pipeline variants under comparison (\S\ref{sec:pipeline-variants}). We then describe two framework versions: an initial version using GPT-4.1-mini that showed important limitations (\S\ref{sec:initial-framework}), and the final version using Claude Sonnet~4.6 that fixes those limitations (\S\ref{sec:final-framework}). We end with the significance testing method (\S\ref{sec:significance-testing}).
...
\item[No-ontology RAG.] This variant uses the same improved Refinement and Generation Agent prompts as the ontology-enhanced pipeline, but the Refinement Agent receives no ontology. It evaluates documents based on the question text alone. This ablation isolates the ontology's contribution from prompt engineering improvements.
...
Our initial evaluation used GPT-4.1-mini as the judge with five criteria on a 1--5 scale, following the setup of the DLR baseline~\citep{elbaff2025earthobservationrag}. Two problems emerged. First, score saturation: most answers clustered at 5, leaving too little room to distinguish between pipeline variants. Second, the five criteria borrowed from the baseline (Factuality, Relevance, Groundedness, Helpfulness, and Depth) do not directly measure the qualities this thesis aims to improve: directness, scope awareness, quantitative precision, and structured response organisation.

\subsection{Final Framework}
\label{sec:final-framework}

The final framework replaces GPT-4.1-mini with Claude Sonnet~4.6 and expands to eight criteria on a 1--10 scale, avoiding the ceiling effects that limited the initial framework. We switched to Claude Sonnet~4.6 to remove self-enhancement bias. The generation model is from the Mistral family, which is not an Anthropic model, so a Claude-based judge cannot favour its own outputs.
...
The four thesis-specific criteria (Directness, Quantitative Precision, Response Structure, and Scope Awareness) were not part of the initial GPT-4.1-mini framework (Appendix~\ref{app:initial-criteria}). Full score-band descriptions for all eight criteria are given in Appendix~\ref{app:eval-criteria}.
...
For each question-answer pair, the judge receives the system prompt with criteria definitions and score range descriptions, followed by the question and the answer. It returns eight integer scores. Each pair is evaluated on its own; the judge never sees answers from other pipeline variants for comparison. This single-answer grading approach avoids position bias~\citep{zheng2023judging}. The Directness criterion explicitly penalises unnecessary verbosity.
